...
Android Studio создаёт манифест JS-приложения автоматически на основе файла client.yaml, который размещён по адресу `app\assets`.
...
В списке `capabilities` требуется перечислить разрешения, которые потребуются вашему приложению.

Доступны следующие разрешения:

* `internet` – позволяет приложению выходить в интернет, например, [чтобы обратиться к стороннему сервису](./doc_js_third_party_service_communication.html).
* `devices` – позволяет приложению получать данные от устройств, подключённых к смарт, терминалу, например [данные от весов](./doc_js_devices.html#Scales).
* `storage` – позволяет приложению [хранить данные на смарт-терминале](./doc_js_storage_api.html), между запусками приложения.
* `barcode-scanner` – позволяет приложению получать [данные от сканера штрихкодов](./doc_js_devices.html#BarcodeScanner).
...
С помощью плагинов приложение можно подписать на события, которые рассылает смарт-терминал. Смарт-терминал создаёт события перед применением изменений, смарт-терминал ждёт ответ от приложений, например, [добавление позиции в чек](./doc_js_receipt_interactions.html)
Параметры элемента списка:
